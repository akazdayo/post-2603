  ...
  #grid(
    ...
    [
      #section(
        [主張],
        [
          一般的なWebカメラだけで、VRChatにおける腰・脚の位置変化をアバターへ反映するところまでは実現できた。単眼映像から3次元姿勢を推定し、腰と両脚の情報をVRChat向けのトラッキング信号へ変換することで、フルトラッキングらしい基本的な動きを再現できた。
          #parbreak()
          一方で、回転情報の不足と処理遅延が残り、物理トラッカーの完全な代替には至っていない。本研究では、一般的なWebカメラのみでどこまで身体追従が可能かを示すとともに、実用化に向けた課題を整理した。
        ],
      )
